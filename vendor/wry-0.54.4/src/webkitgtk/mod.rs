...
impl InnerWebView {
    ...
    W: IsA<gtk::Container>,
  {
    ...
    if let Some(input_context) = webview.input_method_context() {
      let input_context_focus_in = input_context.clone();
      webview.connect_focus_in_event(move |_view, _event| {
        input_context_focus_in.notify_focus_in();
        gtk::glib::Propagation::Proceed
      });

      let input_context_focus_out = input_context.clone();
      webview.connect_focus_out_event(move |_view, _event| {
        input_context_focus_out.notify_focus_out();
        gtk::glib::Propagation::Proceed
      });

      let input_context_grab = input_context.clone();
      webview.connect_grab_focus(move |_view| {
        input_context_grab.notify_focus_in();
      });
    }
  ...
  fn set_webview_settings(webview: &WebView, attributes: &WebViewAttributes) {
    // Keep WebKitGTK preedit enabled for Wayland + IBus so CJK IME composition
    // works correctly. Wry disables this globally to improve fcitx cursor anchoring,
    // but that breaks Chinese input on some Wayland/IBus setups.
    if let Some(input_context) = webview.input_method_context() {
      let xdg_session_type = std::env::var("XDG_SESSION_TYPE").unwrap_or_default().to_lowercase();
      let gtk_im_module = std::env::var("GTK_IM_MODULE").unwrap_or_default().to_lowercase();
      let qt_im_module = std::env::var("QT_IM_MODULE").unwrap_or_default().to_lowercase();
      let xmodifiers = std::env::var("XMODIFIERS").unwrap_or_default().to_lowercase();

      let uses_ibus = gtk_im_module.contains("ibus")
        || qt_im_module.contains("ibus")
        || xmodifiers.contains("@im=ibus");
      let keep_preedit = xdg_session_type == "wayland" && uses_ibus;

      input_context.set_enable_preedit(keep_preedit);
    }
